...
import { addVin, searchVin } from "@/lib/actions";
import {
  Form,
  FormControl,
  FormField,
...
export default function SearchForm() {
  ...
  const searchForm = useForm<z.infer<typeof Vin>>({
    resolver: zodResolver(Vin),
    defaultValues: {
      version: "",
      equipment_code: "000",
      year_of_issue: "",
  ...
  const addForm = useForm<z.infer<typeof AddedVin>>({
  ...
  const onSubmit = async (formData: z.infer<typeof Vin>) => {
    try {
      setVinNumber("");
      const formDataToSend = new FormData();
      formDataToSend.append("version", formData.version);
      formDataToSend.append("equipment_code", formData.equipment_code);
      formDataToSend.append("year_of_issue", formData.year_of_issue);
      formDataToSend.append(
        "place_of_production",
        formData.place_of_production
      );
      const response = await searchVin(
        { errors: {}, message: "" },
        formDataToSend
      );
      setSearchResults(response.data);
      addForm.setValue("serial_number", response.data.next_serial_number);
      addForm.setValue("version", formData.version);
      addForm.setValue("equipment_code", formData.equipment_code);
      addForm.setValue("year_of_issue", formData.year_of_issue);
      addForm.setValue("place_of_production", formData.place_of_production);
      setVinNumber(
        `${formData.version}${formData.equipment_code}${formData.year_of_issue}${response.data.next_serial_number}${formData.place_of_production}`
      );
      toast.success("Available serial number found", {
  ...
  const onSubmitAdd = async (formData: z.infer<typeof AddedVin>) => {
    try {
      const formDataToSend = new FormData();
      formDataToSend.append("version", formData.version);
      formDataToSend.append("equipment_code", formData.equipment_code);
      ...
      formDataToSend.append("serial_number", formData.serial_number.toString());
      const response = await addVin(
        { errors: {}, message: "" },
        formDataToSend
      );
      setSearchResults(response.data);
      setVinNumber("");
      toast.success("Vin added successfully", { position: "bottom-left" });
      setIsAnimating(false);
    } catch (error) {
      toast.error("Adding Vin failed please try again later", {
        position: "bottom-left",
      });
    }
  };
  ...
  return (
    <div className="text-white flex flex-col items-center w-full min-h-full">
      ...
      <div className="w-[450px] h-[200px] mt-[20px] bg-[#1c2b50] p-5 border-2 rounded-xl flex flex-col items-center">
        <Form {...addForm}>
          <form
          ...
          >
            <div className="h-[80px] flex flex-col items-center">
              <div className="flex gap-2 mt-2">
                <motion.div>Availale serial number: </motion.div>
                <motion.div
            ...
            <div>
              <Button
                disabled={!searchResults?.next_serial_number}
                type="submit"
              >
                Add
              </Button>
